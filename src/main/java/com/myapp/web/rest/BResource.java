...
public class BResource {
    ...
    public ResponseEntity<B> createB(@RequestBody B b) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/bs/" + b.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, b.getId().toString()))
            .body(b);
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<B> getB(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteB(@PathVariable("id") Long id) {
